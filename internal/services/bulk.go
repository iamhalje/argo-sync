	...
	for _, t := range targets {
		...
		g.Go(func() error {
			...
			case models.ActionSync:
				...
				if err != nil {
					// If a sync is already running, we can still wait for it to finish (useful for "sync again"
					// when an operation is already in progress).
					if opts.Wait && isOpInProgressErr(err) {
						emit(models.ProgressEvent{At: time.Now(), Target: t, Phase: models.TaskRunning, Action: action, Message: "sync already in progress; switching to wait"})
						err = nil
					} else {
						break
					}
...
func isOpInProgressErr(err error) bool {
	// Argo CD typically returns:
	// "rpc error: code = FailedPrecondition desc = another operation is already in progress"
	if err == nil {
		return false
	}
	s := strings.ToLower(err.Error())
	return strings.Contains(s, "another operation is already in progress") ||
		strings.Contains(s, "operation is already in progress")
}
